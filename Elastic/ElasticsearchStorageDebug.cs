...
namespace ClerkBot.Elastic
{
    ...
    public class ElasticsearchStorageDebug : IStorage
    {
        ...
        public ElasticsearchStorageDebug(ElasticsearchStorageOptionsDebug elasticsearchStorageOptions)
        {
            ...
            indexName = elasticsearchStorageOptions.IndexName ?? throw new ArgumentNullException($"Index name for Elasticsearch is required.", nameof(elasticsearchStorageOptions.IndexName));
            indexMappingDepthLimit = elasticsearchStorageOptions.IndexMappingDepthLimit ?? 100000;
        ...
        public async Task WriteAsync(IDictionary<string, object> changes, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var (key, newValue) in changes)
            {
                ...
                await elasticClient.IndexAsync(documentItem, i => i
                    .Index(indexToUse).Refresh(Refresh.True), cancellationToken).ConfigureAwait(false);
            }
        }
        ...
        private void CreateClient(IConnectionPool connectionPool)
        {
